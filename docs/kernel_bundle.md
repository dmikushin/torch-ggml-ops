...
1. Enumerate entries in a stable reviewed order.
2. Hash all build inputs used for cache invalidation:
   - compiler identity;
...
1. Read the selected artifact into retained process memory.
2. Call `hipModuleLoadData`.
3. Resolve the expected symbol with `hipModuleGetFunction`.
4. Cache retained bytes, module, and function.
5. Launch on PyTorch's current stream with `hipModuleLaunchKernel`.
...
6. Add correctness coverage and any required resource gate.
7. Run reproducibility, packaging, installed-wheel, and performance validation.
